...
<div class="relative" x-data="{ {{ $func }}: false }">
    <input {!! $attributes->merge([
        'class' => 'border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm',
    ]) !!}
    x-bind:type="{{ $func }} ? 'text' : 'password'"
    >
    <button type="button" id="{{ $func }}"  x-on:click="{{ $func }} = !{{ $func }}"
        class="absolute top-0 end-0 p-3.5 rounded-e-md dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
        <svg class=" text-gray-400 dark:text-neutral-600" width="24" height="24">
